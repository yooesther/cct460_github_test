            ...
            switch ( $view_days_num ) {
                ...
                case '1':
                    if ( empty( $this->request_args['scroll_start_date'] ) )
                        $start_day = intval( date( "d" ) );                               // Today date
                    ...
                    $wh_booking_date = date( "Y-m-d", $real_date );

                    $real_date = mktime( 0, 0, 0, $start_month, ($start_day + 0 + $scroll_day ), $start_year );
                    $wh_booking_date2 = date( "Y-m-d", $real_date );
                    break;

                case '7':
                    if ( empty( $this->request_args['scroll_start_date'] ) )
                        $start_day = intval( date( "d" ) );   //Today  date
                    $start_week_day_num = intval( date( "w" ) );
                    ...
                    if ( $start_week_day_num != $start_day_weeek ) {
                        for ( $d_inc = 1; $d_inc < 8; $d_inc++ ) {              // Just get week  back
                            ...
                            $start_week_day_num = intval( date( "w", $real_date ) );
                            if ( $start_week_day_num == $start_day_weeek ) {
                                $start_day   = intval( date( "d", $real_date ) );
                                $start_year  = intval( date( "Y", $real_date ) );
                                $start_month = intval( date( "m", $real_date ) );
                                $d_inc = 9;
                            }
                        }
                    }
    ...
    public function wpbc_show_timeline_header_row( $start_date = false ) {
        ...
        if ( $start_date === false ) {
            ...
            if ( ! empty( $this->request_args['scroll_start_date'] ) )           
                ...
                list( $start_year, $start_month, $start_day ) = explode( '-', date( 'Y-n-j' ) );
            
        } else {
                list( $start_year, $start_month, $start_day ) = explode( '-', date( 'Y-m-d', $start_date ) );
        }
                             ...
                             style="<?php echo 'width:' . $cell_width . ';'; ?>" ><?php 
                                ...
                                for ( $tt = 0; $tt < $time_selles_num; $tt++ ) { ?>
                                    ...
                                    <div class="wpbc_time_section_in_day wpbc_time_section_in_day_header time_hour<?php echo ($tt*$tm); 
                                             ?> wpbc_time_in_days_num_<?php echo $view_days_num;?>"><?php 
